...
class Verification extends Model
{
    ...
    public static function gettotalbytype($jid,$type)
    {
        $t=DB::table('verification_surveys')
        ->where('job_id',$jid)
        ->where('surveyor_billing',$type)
        ->where('status',1)
        ->select(
    ...
    public static function invoiceDetails($id,$bill)
    {
        $recs = DB::select('call GetInvoiceDetailsMezzan(?)', array($id));
        

        $myDate =$bill[0]->bill_date;
        $attenname =$bill[0]->name;
        ...
        foreach($recs as $rec)
        {
            ...
            // $tregular=DB::table('verification_surveys')
            // ->where('job_id',$rec->jid)
            ...
            // $tlong=DB::table('verification_surveys')
            // ->where('job_id',$rec->jid)
            ...
            $totalregular+=Verification::gettotalbytype($rec->jid,"Regular");
            $totallong+=Verification::gettotalbytype($rec->jid,"Long");
            $totalextra+=Verification::gettotalbytype($rec->jid,"Extra Long");
            $snaps+=$rec->snaps>0?1:0;
            $tamount+=$rec->total_amount;
 
        $regular = DB::select('call GetMezzanVisitDetails(?,?)', array("Regular",$rec->jid));
        $long = DB::select('call GetMezzanVisitDetails(?,?)', array("Long",$rec->jid));
        $extra = DB::select('call GetMezzanVisitDetails(?,?)', array("Extra Long",$rec->jid));


                if(isset($regular[0]->Regular))
                {
                    $rg=str_replace(',','<br>',$regular[0]->Regular);

                }
                else{
                    $rg=str_replace(',','<br>',"0,0");

                }

                if(isset($long[0]->Regular))
                {
                    $lg=str_replace(',','<br>',$long[0]->Regular);

                }
                else{
                    $lg=str_replace(',','<br>',"0,0");

                }

                if(isset($extra[0]->Regular))
                {
                    $exl=str_replace(',','<br>',$extra[0]->Regular);


                }
                else{
                    $exl=str_replace(',','<br>',"0,0");
            ...
            $items->push([
                ...
                'applicant_cnic'=>$rec->applicant_cnic,
                'type'=>str_replace(',','<br>',$rec->type),
                'city'=> str_replace(',','<br>',$rec->city),
                'above'=>"",
                'regular'=>str_replace('0','<br>',$rg),
                'log'=>str_replace('0','<br>',$lg),
                'extra_long'=>str_replace('0','<br>',$exl),
                'snaps'=>$rec->snaps>0?'1':'',
                'total_amount'=>number_format($rec->total_amount),
            ]);
        ...
        // $visits=DB::select('call GetInvoiceDetailsMezzan(?)', array($id));
        ...
        $data=[
            ...
            $totalextra,
            $snaps,
            number_format($tamount),
        ];
        $pdf1 = new PDF(['mode' => 'utf-8', 'format' => 'A3-L']);

        $pdf1 = PDF::loadView('Verification/verinvoicemezzan',compact('data'));
        // $pdf1->setPaper('A3', 'landscape');
        $pdf1->save('Reports/Verification/mezzaninvoicepdf.pdf');

        $pdf = new \setasign\Fpdi\Fpdi('P','mm',array(297,420));
        $pageCount1=$pdf->setSourceFile("Reports/Verification/mezzaninvoicepdf.pdf");
    ...
    public static function updateServiceCharges($request,$id)
    {
        $newchar=$request->newcharges;
        $oldchar=$request->oldcharges;
        ...
        if($newchar>$oldchar)
        {
            ...
            if($oldchar==0)
            {
                ...
                foreach($surveys as $survey)
                {
                    ...
                    DB::table('verification_surveys')
                    ->where('id',$survey->id)
                    ->update([
                        'service_charges'=>$survey->service_charges+$rem
                    ]);
                }
    ...
    // public s function getSurveysContainingValue()
    public static function updateTotalCharges($request,$id)
    {
        ...
        foreach($surveys as $survey)
        {
            $total=$survey->service_charges+$survey->snap_charges;
        ...
        ->select('verification_jobs.service_charges','verification_jobs.atten','sales_tax.rate')
        ->get();
        ...
        if($job[0]->rate=='')
        {
            $rate=$totalcharges*(0/100);
            $totalamount=$rate+$totalcharges;
        }   
        
        else {
            # code...

            $rate=$totalcharges*($job[0]->rate/100);
            $totalamount=$rate+$totalcharges;
        }
    ...
    public static function updateAttentionandcharges($request,$jid,$id)
    {
        ...
        DB::table('verification_jobs')->where('id',$jid)->update([
            'atten'=>$request->attention
        ]);
        ...
        ->where('bill.id',$id)
        ->select('bill.bill_date','c_bank.bank_code')
        ->get();

        DB::table('bill')->where('id',$id)->update([
            'atten'=>$request->attention,
            'invoice_id'=>'KGT' .'/VER/'.$bill[0]->bank_code.'/'.$request->attention.'/'.date_format(date_create($bill[0]->bill_date),'m').'/'.date_format(date_create($bill[0]->bill_date),'Y')
        ]);
        ...
        ->leftJoin('c_atten','verification_jobs.atten','=','c_atten.id')
        ->leftJoin('sales_tax','c_atten.sale_reg','=','sales_tax.region')
        ->where('verification_jobs.id','=',$jid)
        ->select('verification_jobs.service_charges','verification_jobs.atten','sales_tax.rate')
        ->get();
    ...
    public static function updateSnapCharges($request,$id)
    {
        $newchar=$request->newchargessnap;
        $oldchar=$request->oldchargessnap;
        ...
        if($newchar>$oldchar)
        {
            ...
            if($oldchar==0)
            {
                ...
                foreach($surveys as $survey)
                {
                    ...
                    ->where('id',$survey->id)
                    ->update([
                        'snap_charges'=>$survey->snap_charges+$rem
                    ]);
    ...
    public static function getBillsOfInvoice($request)
    {
        ...
        ->where('bill.service','Verification')
        ...
        ->leftJoin('c_atten','verification_jobs.atten','=','c_atten.id')
        ->leftJoin('sales_tax','c_atten.sale_reg','=','sales_tax.region')
        ->leftJoin('verification_surveys','verification_jobs.id','=','verification_surveys.job_id')
        ->select(
            'bill.id',
            'verification_jobs.id as job_id',
            'bill.bill_number',
            'c_bank.bank_name',
            'c_branch.branch_name',
            'verification_jobs.product_type',
            DB::Raw('SUM(verification_surveys.service_charges) as service_charges'),
            DB::Raw('SUM(verification_surveys.snap_charges) as snap_charges'),
            'bill.tax',
            'bill.total_amount',
            'sales_tax.rate',
            'c_atten.name as attention_name',
            'bill.atten as attention_value'
            
            )
        ->groupBy('verification_surveys.job_id')
        ->get();
    }
    ...
    public static function printInvoice($id)
    {
        ...
        ->where('verification_jobs.product_type', 'Auto')
        ->select(
            DB::raw('*')
            )
        ->get();
        ...
        ->leftJoin('verification_jobs', 'bill.bill_number', '=', 'verification_jobs.job_id')
        ->where('bill.invoice_id', $id)
        ->where('verification_jobs.product_type', 'Home')
        ->select(
            DB::raw('*')
            )
        ->get();
        ...
        $hm=count($home);
            ...
            if($at != "0" && $hm != "0")
            {
                ...
                $data=Verification::getAutoInvoice($id,'Auto');

                $pdf1 = PDF::loadView('Verification/verinvoice',compact('data'));
                $pdf1->save('Reports/Verification/autoinvoice.pdf');
    
    
                $data=Verification::getAutoInvoice($id,'Home');
    
                $pdf1 = PDF::loadView('Verification/verinvoice',compact('data'));
                $pdf1->save('Reports/Verification/homeinvoice.pdf');
    
                $pdf = new \setasign\Fpdi\Fpdi('P', 'mm', array(210, 297));
    
                $pdf->AddPage();
                $pdf->setSourceFile("Reports/Verification/autoinvoice.pdf");
                $tplIdx = $pdf->importPage(1);
                $pdf->useTemplate($tplIdx, 5, 5);
    
                $pdf->AddPage();
                $pdf->setSourceFile("Reports/Verification/homeinvoice.pdf");
                $tplIdx = $pdf->importPage(1);
                $pdf->useTemplate($tplIdx, 5, 5);
    ...
    public static function printInvoiceFHM($id)
    {
        ...
        //     $exl=str_replace(',','<br>',$extra[0]->Regular);
            $items->push([
                'applicant_name'=>$rec->applicant_name,
                'bill_date'=>date_format(date_create($rec->bill_date),'d-M-Y'),
                'type'=>str_replace(',','<br>',$rec->type),
                'city'=> str_replace(',','<br>',$rec->city),
                'total_amount'=>number_format($rec->total_amount),
            ]);
        ...
        $data=[
            $id,
            $date,
            Verification::convertNumberToWord($total[0]->total_amnt) .' only',
            $total[0]->total,
            $sales[0]->tax,
            $salesrate[0]->rate,
            $total[0]->total+$sales[0]->tax,
            $items


        ];

        $pdf1 = PDF::loadView('Verification/verinvoicefhm',compact('data'));
        $pdf1->save('Reports/Verification/fhminvoice.pdf');


        $pdf->AddPage();
        $pdf->setSourceFile("Reports/Verification/fhminvoice.pdf");
        $tplIdx = $pdf->importPage(1);
        $pdf->useTemplate($tplIdx, 5, 5);
    ...
    public static function printInvoiceHMM($id)
    {
        ...
        //     $exl=str_replace(',','<br>',$extra[0]->Regular);
            ...
            $items->push([
                ...
                'type'=>str_replace(',','<br>',$rec->type),
                'city'=> str_replace(',','<br>',$rec->city),
                'total_amount'=>str_replace(',','<br>',$rec->total_amount),
            ]);
        ...
        $pdf1 = PDF::loadView('Verification/verinvoicehmm',compact('data'));
        $pdf1->save('Reports/Verification/hmminvoice.pdf');


        $pdf->AddPage();
        $pdf->setSourceFile("Reports/Verification/hmminvoice.pdf");
        $tplIdx = $pdf->importPage(1);
        $pdf->useTemplate($tplIdx, 5, 5);
    ...
    public static function printInvoiceDIB($id)
    {
        ...
        $pdf1 = PDF::loadView('Verification/verinvoicetrelis',compact('data'));
        $pdf1->save('Reports/Verification/trelisinvoice.pdf');


        $pdf->AddPage();
        $pdf->setSourceFile("Reports/Verification/trelisinvoice.pdf");
        $tplIdx = $pdf->importPage(1);
        $pdf->useTemplate($tplIdx, 5, 5);
    ...
    public static function printInvoiceTrelis($id)
    {
    ...
    public static function verticalAutoFormatBislami($request,$myarray,$billid)
    {
        ...
        foreach($myarray as $item)
        {
            for ($i=2; $i < $count ; $i++) { 
                ...
                // $refid="";
                // echo $item[$i][4]."<br>";
                Verification::storeBulkVerification($request,$cnid,$refid);
                ...
                $amc= $item[$i][15];
                $bs= $item[$i][16];
                $salaryslip= $item[$i][17];
                $no= $item[$i][18];
                $other= $item[$i][19];

                $teamlead= $item[$i][20];
                Verification::addSurveysBulk($request, $cid,$radd,$offadd,$amc,$bs,$salaryslip,$no,$other,$teamlead);

                $appname= $item[$i][6];
                $appcell= $item[$i][8];

                Verification::addDetailsBulk($request, $cid,$appname,$appcell);
        ...
        foreach($myarray as $item)
        {
            ...
            for ($i=2; $i < $count ; $i++) { 
                ...
                $offarea= $item[$i][15];
                 ...
                 $teamlead= '';
                 $business=$item[$i][5];
                 Verification::addSurveysBulkForSoneri($request, $cid,$radd,$offadd,$amc,$bs,$salaryslip,$no,$other,$teamlead,$business);
    ...
    public static function verticalHomeFormatBislami($request,$myarray)
    {
        ...
        $billid = Verification::getLatestJob($request);
            ...
            for ($p=0; $p < count($recs) ; $p++) { 
                ...
                if($recs[$p][0][5]!="")
                {
                    ...
                    $appcell=$recs[$p][0][7];
                    $appdes=$recs[$p][0][8];
                    $appbusiness=$recs[$p][0][9];
                    $apprescell="";
                    $appworkcell= "";

                    Verification::addDetailsBulkforhome($request, $cid,$appname,$appcell,$appfather,$appdob, $appcnic,$apprescell,$appworkcell,$appdes,$appbusiness);
                    ...
                    if($recs[$p][0][9]!=null)
                    {
                        ...
                        $offadd= $recs[$p][0][10];
                        $teamlead= $recs[$p][0][17];
                        $salaryslip= $recs[$p][0][14]==null ? 0 : 1;
                    ...
                    if($recs[$p][0][12]!=null )
                    {
                        ...
                        $radd= $recs[$p][0][12];
                        $teamlead= $recs[$p][0][17];
                        $surname= "Account Maintanance Certificate";
                        Verification::addResSurveyforhome($request, $cid,$radd,$teamlead,$city,$surname);
                ...
                if($recs[$p][2][5]!="")
                {
                    ...
                    if($recs[$p][2][9]!=null)
                    {
                        ...
                        $offadd= $recs[$p][2][10];
                        $teamlead= $recs[$p][2][17];
                        $salaryslip= $recs[$p][2][14]==null ? 0 : 1;
                ...
                if($recs[$p][3][5]!="")
                {
                    ...
                    if($recs[$p][3][9]!=null)
                    {
                        ...
                        $offadd= $recs[$p][3][10];
                        $teamlead= $recs[$p][3][17];
                        $salaryslip= $recs[$p][3][14]==null ? 0 : 1;
        ...
        foreach($myarray as $item)
        {
            ...
            for ($i=2; $i < $count ; $i++) { 
                    ...
                    if($item[$i][14]!=null)
                    {
                        ...
                        // $areares= $item[$i][9];
                         $area= $item[$i][15];
                        Verification::addResSurveyforhomeSoneri($request, $cid,$radd,$teamlead,$city,$surname,$area);
                    ...
                    if($item[$i][8]!=null)
                    {
                        ...
                        Verification::addWorkSurveyforhomeSoneri($request, $cid,$offadd,$teamlead,$salaryslip,$area);
    ...
    //                 $type="Applicant";
    //                 Verification::storeBulkVerificationforhome($request,$cnid,$type);
    //                 $cjob = Verification::getLatestJob($request);
    //                 $cid = $cjob->id;
    //                 $cnid=$cjob->id;
    //                 Verification::addCommonJob($request, $cid,$billid->id);
    ...
    //                 Verification::addDetailsBulkforhome($request, $cid,$appname,$appcell,$appfather,$appdob, $appcnic,$apprescell,$appworkcell);
    ...
    //                     Verification::addResSurveyforhome($request, $cid,$radd,$teamlead,$city);
    ...
    //                 Verification::addWorkSurveyforhome($request, $cid,$offadd,$teamlead);
         ...
         foreach($myarray as $item)
            {
                for ($i=1; $i < $count ; $i++) { 
                    ...
                    if($appl!="")
                    {
                        ...
                        Verification::addSurveysBulkforhome($request, $cid,$radd,$offadd,$teamlead);
                    ...
                    if($coapp1!="")
                    {
                        ...
                        Verification::addCommonJob($request, $cid,$billid->id);
                        Verification::updateJobID($request, $cid);
                        // Verification::addDetailsBulkforhome($request, $cid,$appname,$appcell,$appfather,$appdob, $appcnic);
    ...
    public static function updateVerification($request, $id)
    {
        ...
        $ibr = Verification::where('id', $id)->first();
    ...
    public static function orderImage($request)
    {
        $val = DB::table('verification_survey_images')->where('id', $request->id)->update([
    ...
    public static function getAutoInvoice($id,$type)
    {
        ...
        $date = Carbon::createFromFormat('Y-m-d', $myDate)
            ...
            $data=[
                ...
                Verification::convertNumberToWord($total[0]->total_amnt) .' only', //Total
    ...
    public static function addSurveysBulk($request,$id,$rad,$oad,$amc,$bs,$salaryslip,$no,$other,$lead)
    {
    ...
    public static function addDetailsBulkForSoneri($request, $cid,$apname,$appcell,$cnic,$business)
    {
        ...
        $val = DB::table('verification_job_details')->insert([
            ...
            'applicant_res_phone' => $request->applicant_res_phone,
            'applicant_work_phone' => $request->applicant_work_phone,
            'applicant_business_name' => $business
        ]);

    }
    public static function addDetailsBulk($request, $cid,$apname,$appcell)
    {
    ...
    public static function updateJobID($request, $cid)
    {
        ...
        $val1 = Verification::findOrFail($cid);
    ...
    {
        $pdf->setSourceFile("src/reportformat/islami/workplaceperfoma.pdf");
        ...
        if($survey->wp_permisses_is="Others")
        {
         ...
         $pdf->Write(0,  $survey->surveyor);
           ...
           $recs=count($job);
              ...
              // $pdf->SetTextColor(255, 0, 0);
              $pdf->SetXY(11, 35);
              $pdf->Write(0, 'BankIslami Pakistan Limited');
    ...
    public static function applicantResidenceReportforhome($request,$pdf,$survey,$id)
    {
        ...
        $pdf->setSourceFile("src/reportformat/islami/residenceperfoma.pdf");
    ...
    {
        $pdf->setSourceFile("src/reportformat/islami/workplacecoperfoma.pdf");
        $tplIdx = $pdf->importPage(1);
        $pdf->useTemplate($tplIdx, 0, 0);
    ...
    public static function applicantcoResidenceReportforhome($request,$pdf,$survey,$id,$appinfo)
    {
        ...
        $pdf->setSourceFile("src/reportformat/islami/residencecoperfoma.pdf");
    ...
    public static function applicantrefWorkplaceReportforhome($request,$pdf,$survey,$id,$appinfo)
    {
        ...
        $pdf->setSourceFile("src/reportformat/islami/workplacereferenceproforma.pdf");
    ...
    public static function applicantrefResidenceReportforhome($request,$pdf,$survey,$id,$appinfo)
    {
        ...
        $pdf->setSourceFile("src/reportformat/islami/residencereferenceproforma.pdf");
