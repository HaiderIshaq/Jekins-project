...
class IEController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $user = IE::getUserCompany($uid, $token);
        return view('IE.create', compact('user'));
    ...
    public function addJob(Request $request)
    {
        ...
        IE::storeMuccaduum($request);
        $cjob = IE::getLatestJob($request);
        $cid = $cjob->id;
        IE::addCommonJob($request, $cid);
        IE::updateJobID($request, $cid);

    }
    public function updateIEJob(Request $request,$id)
    {

        IE::updateIE($request, $id);
        IE::updateLocalJob($request, $id);

    }

    public function getIeJob($id)
    {
        $jobDetails = IE::getJobById($id);
        return $jobDetails;
    }


    public function printIEBill(Request $request)
    {
        ...
        if (empty($bill)) {
            ...
            $log->service = 'Income Estimation|Livestock';
            $log->save();


            $billData = IE::getJobByIdForBill($id, $request->jobid);

            $tonumber = IE::convertNumberToWord($total);
            ...
            $pdf1 = PDF::loadView('IE/insinvoice', compact('data'));
            $pdf1->save('Reports/IE/' . $id . '/invoice.pdf');
            $pdf = new \setasign\Fpdi\Fpdi('P', 'mm', array(210, 297));

            $pdf->AddPage();
            $pdf->setSourceFile("Reports/IE/$id/invoice.pdf");
