...
class IE extends Model
{
    ...
    {
        $cjob = DB::table('ie_jobs')->latest('created_at', 'desc')->first();
        Storage::disk('dir')->makeDirectory('Reports/IE/' . $cjob->id . '/Report');
    ...
    {
        $job = DB::table('ie_jobs')
            ->where('ie_jobs.id', $id)
            ->leftJoin('c_company', 'ie_jobs.job_by', '=', 'c_company.company_id')
            ->leftJoin('c_region', 'ie_jobs.region_id', '=', 'c_region.reg_id')
            ->leftJoin('c_muccaduum', 'ie_jobs.muccaduum_id', '=', 'c_muccaduum.id')
            ->leftJoin('c_city AS region', 'c_region.reg_city_id', '=', 'region.city_id')
            ->leftJoin('c_region AS op', 'ie_jobs.operational_branch', '=', 'op.reg_id')
            ->leftJoin('c_city AS opbr', 'op.reg_city_id', '=', 'opbr.city_id')
            ->leftJoin('c_bank', 'ie_jobs.bank_id', '=', 'c_bank.bank_id')
            ->leftJoin('ibr_vendors', 'ie_jobs.byvendor_id', '=', 'ibr_vendors.id')

            ->leftJoin('c_customer', 'ie_jobs.customer_id', '=', 'c_customer.cust_id')
            ->leftJoin('tax_regions', 'ie_jobs.sale_reg', '=', 'tax_regions.id')
            ->leftJoin('c_branch', 'ie_jobs.branch_id', '=', 'c_branch.branch_id')
            ->leftJoin('c_employees', 'ie_jobs.conducted_by', '=', 'c_employees.id')
    ...
    public static function updateIE($request, $id)
    {
        date_default_timezone_set("Asia/Karachi");

        $ibr = IE::where('id', $id)->first();
        ...
        $log->date = date('Y/m/d');
        $log->time = date("h:i:sa");
        $log->service = 'Income Estimations';
        $log->save();
    }
    ...
    public static function addCommonJob($request, $cid)
    {
        ...
        if ($givenBy == "Bank") {
            ...
            $bill->service = 'Income Estimation';
            $bill->save();
        } else {
